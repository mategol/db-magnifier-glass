            ...
            for hit in result.stdout.split('\n'):
                if hit != '':
                    with open(hit.split(':')[0], 'rb') as file:
                        ...
                        hit_context = hit_before + file.read(300).decode('utf-8').replace(command_options['goal'], f'\033[31;1m{command_options["goal"]}\033[0m')

                    print(f'\033[33m{hit.split(":")[0]}:\033[32m{hit.split(":")[1]}\033[0m:\033[35m{hit.split(":")[2]}\033[0m:{hit_context[:hit_context.find("\n")]}')

            ...
            for hit in result.stdout.split('\n'):
                if hit != '':
                    with open(hit.split(':')[0], 'rb') as file:
                        file.seek(int(hit.split(':')[2])-30)
                        hit_before = file.read(30).decode('utf-8', errors='replace')
                        hit_before = hit_before[::-1][hit_before.find('\n')+1:][::-1]
                        hit_context = hit_before + file.read(30).decode('utf-8', errors='replace').replace(command_options['goal'], f'\033[31;1m{command_options["goal"]}\033[0m')
